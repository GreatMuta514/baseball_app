...
RSpec.describe ProGamePredict, type: :model do
  ...
  let(:pro_game_predict) { FactoryBot.build(:pro_game_predict, user:, pro_game:) }
  ...
  describe 'スコアのバリデーションチェック' do
    let(:too_many_score_predict) { FactoryBot.build(:pro_game_predict, :too_many_score, user:, pro_game:) }
    let(:not_integer_score_predict) { FactoryBot.build(:pro_game_predict, :not_integer_score, user:, pro_game:) }
    let(:decimal_score_predict) { FactoryBot.build(:pro_game_predict, :decimal, user:, pro_game:) }
    let(:negative_score_predict) { FactoryBot.build(:pro_game_predict, :negative_score, user:, pro_game:) }
